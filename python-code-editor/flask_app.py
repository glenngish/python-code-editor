...
SENDING_EMAIL = "sunshinerug215@gmail.com"
...
def signin(department):
    ...
    if request.method == 'POST':
        ...
        # TODO: Changing sending email to recipient_email
        msg = Message('New Store Representative Sign-In',
                      sender='bfurn1589@outlook.com',
                      recipients=[SENDING_EMAIL])
        #              recipients=[recipient_email])
        msg.html = f"""
		<p>Hello,</p>
		<p>This is to notify you that {rep_name} from {vendor_message} has signed in as a {department} Representative at the {selected_store} location.</p>
		<p>Summary of Visit: {visit_summary}</p>

		<p>Please confirm the visit by clicking the button below: </p>
		<form action="{request.host_url}confirm/{new_signin.id}" method="POST">
			<input type="submit" value="Confirm Visit">
		</form>
        
		<p>Regards,</p>
		<p>Baer's Furniture Rep Update</p>
		"""

        try:
            mail.send(msg)
            print("Sent confirm email successfully")
        except Exception as e:
            print("Error occured when sending confirm email")
        ...
        <style>
            ...
            @media screen and (max-width: 600px) {{
                select {{
                    ...
                    border-radius: 4px; /* Rounded corners, handled well by iOS */
                    -webkit-appearance: none; /* This disables the inner drop-down arrow in iOS */
                    appearance: none;
                    background-image: url('arrow-down-icon.png'); /* Custom arrow */
                    background-repeat: no-repeat;
                    background-position: right 10px center; /* Arrow position */
                    background-size: 12px; /* Suitable size for the custom arrow */
                }}
            }}
        </style>
    </head>
...
def render_signin_page(form_data):
    ...
    print(selected_store, start_date, end_date)
    ...
    formatted_records = [
        {
            ...
            'confirmed': "Confirmed" if record.confirmed else ""
        }
        for record in store_signin_records
    ]
    stores = [
    ...
    <body>
        ...
        <table class="records-table">
            ...
            {''.join(f"<tr><td>{r['name']}</td><td>{r['vendor']}</td><td>{r['date']}</td><td>{r['time']}</td><td>{r['store']}</td><td>{r['department']}</td><td>{r['visit_summary']}</td><td>{r['confirmed']}</td></tr>" for r in formatted_records)}
        </table>
    </div>
    <script>
...
if __name__ == '__main__':
    app.run(debug=False, host='0.0.0.0', port=21211)
